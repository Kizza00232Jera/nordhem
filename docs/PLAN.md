...
> **STATUS (2026-06-15): Steps 1-10 merged & tagged (main at v0.10); Step 10 wrapped (live demo verified, teaching HTML, interview bank, blog moments, this status). Step 9 (studio editor tools) shipped as PR #11: synonyms CRUD + hot-reload (no 43k reindex, since synonyms are query-time), per-query curations (pin/hide, applied at search time), an append-only change-history audit at /studio/relevance/history, and benchmark-before-apply (POST /synonyms/impact scores candidate synonyms against the judged set before they go live). A real bug caught in live testing: a stopword in a multi-word synonym ("chest of drawers") crashed the whole analyzer, fixed with `lenient` + an editor warning. Honest finding: broad synonyms can LOWER nDCG while raising recall. Boost rules deferred (step 7's RankingConfig already exposes field boosts; per-query/click boosts belong with step 15). Step 10 (analytics & resilience) shipped as PR #12, tag v0.10 (5 slices): first-party search events in Postgres (search_performed / result_clicked / zero_result, fired via sendBeacon so they survive lite mode), a /studio/analytics dashboard (top queries, zero-result rate + backlog, CTR by position, latency p50/p95/p99, live-vs-synthetic split), a judgment-driven simulated-traffic generator (position-biased cascade click model, seeded, honestly labelled synthetic), and the circuit breaker (800ms timeout, opens after 2 failures, half-open probe) + Postgres FTS lite-mode fallback + honest banner + /status page. Bug caught in live browser testing: AND-only FTS returned 0 for "oak bed", fixed to OR semantics (176). Green: 65 web unit + 4 new real-PG integration (events/analytics/fts) + 11 search-service simulate unit; typecheck + web lint clean. NEXT: Step 11 (learning loop + AI).**
...
### ✅ Step 9 — Editor tools (the job-post requirement) (done 2026-06-14)
Studio CRUD for: synonyms (Postgres-stored, hot-reloaded into ES), curations (pin/hide products per query) — all with change history (who/when/what). Each change can be re-evaluated against the benchmark BEFORE applying. This is "develop tools in Node.JS to help editors improve search" made literal. Built: synonyms editor + hot-reload (no reindex), curations (query-time pin/hide), change_log audit + History ledger, /synonyms/impact benchmark-before-apply. Boost rules deferred (covered by step 7 RankingConfig + step 15 click loop).
*Teaching: search governance; synonym_graph mechanics (query-time vs index-time); curation patterns; hot-reload vs query-time application.* → `teaching/step-09.1..9.5-*.html` (five per-slice lessons)

### ✅ Step 10 — Analytics & resilience (done 2026-06-15, PR #12, v0.10)
First-party event tracking (search_performed, result_clicked w/ position, zero_result) → Postgres, fired from the storefront via sendBeacon to /api/events so telemetry keeps recording in lite mode. Dashboard at /studio/analytics: top queries, zero-result rate + backlog, CTR by position (impressions-from-distribution math), latency p50/p95/p99, live-vs-synthetic split. Judgment-driven simulated-traffic generator (position-biased cascade click model, seeded PRNG, honestly labelled source='synthetic'; `pnpm -F @nordhem/search simulate-traffic`). Circuit breaker (800ms timeout, opens after 2 failures, half-open probe after a cooldown) + Postgres full-text "lite mode" fallback (OR-semantics FTS) + honest banner + /status page. Caught + fixed in live testing: AND-only FTS returned 0 for "oak bed" (OR semantics now returns 176).
*Teaching: search KPIs; position bias; circuit breakers & graceful degradation; query profiling.* → `teaching/step-10-analytics-resilience.html`

### ✅ Step 11 — The learning loop + AI (done 2026-06-17, branch step/11-learning-loop-ai)
(a) **Click-feedback ranking**: `aggregate-clicks` reads logged clicks, applies position-bias (inverse-propensity) correction, normalises per query to a bounded (0,1] affinity, writes `click_affinity`; the search builder injects a capped per-product `function_score` boost at query time (lexical). Before/after eval lifted nDCG@10 0.6560→0.6762 (honestly flagged circular: synthetic clicks come from the judgments). Enhancements: a min-observations threshold (don't move ranking on one click) and a recency half-life (trending). Studio `/studio/relevance/affinities`. (b) **AI editor assistant**: `search_suggestion` approval queue (`/studio/relevance/suggestions`); approve → creates a synonym rule via the step-9 pipeline (still has to be Applied), reject discards. Sources: a trigram heuristic CLI that VERIFIES against the live engine (so it never proposes what the stemmer/fuzziness already solve), and an automated LLM generator ("Generate with AI") plus the manual Claude-session path. (c) **Shopping chatbot**: provider-agnostic tool-use over `/search` (`search_products`), the LLM only searches and summarises, never ranks, never in the hot path; configured from `/studio/settings` (off / api key / localhost subscription via the claude CLI), hidden when unconfigured or in lite mode. Decisions D48 (AI strategy: no paid API, dev-time AI on the subscription, provider-agnostic key field). Teaching: `teaching/step-11.1/11.2/11.3-*.html` (pushed to the learning repo). Green: search unit 100, web unit 100, integration (affinity PG+ES, suggestions PG). NOT yet squash-merged/tagged v0.11.
*Teaching: implicit feedback & position bias correction; LLM tool-use architecture; why LLMs stay out of the hot path.* → `teaching/step-11.1-learning-loop.html`, `step-11.2-ai-editor-assistant.html`, `step-11.3-llm-tool-use.html`

### 🟦 Step 12 — Deploy & presentation (deploy LIVE 2026-06-17; presentation artifacts pending)
...
SHIPPED: deployed to Vercel; Neon provisioned via the Vercel storage integration; catalog migrated by pg_dump dev -> Neon (unpooled); the site is LIVE at nordhem-web.vercel.app and verified in lite mode (storefront, Postgres-FTS search with the honest banner, PDPs). Code: deploy-readiness (already circuit-breaker safe), RBAC editor allowlist gating /studio (ADMIN_EMAILS, fail-closed in prod), Better Auth trustedOrigins (domain + VERCEL_URL), Cloudflare tunnel script + bring-your-own-engine (per-session https cookie override + optional SEARCH_API_TOKEN Bearer guard on the search service), README + Mermaid diagram + docs/DEPLOY.md. Decision D51. Full mode (named tunnel + SEARCH_API_URL/SEARCH_API_TOKEN on Vercel) is the next live step. STILL OWED (presentation): teaching HTML, interview demo script, lite-mode screen recording, portfolio project page.
