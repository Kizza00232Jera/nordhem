...
> **STATUS (2026-06-15): Steps 1-9 merged & tagged (main at v0.9). Step 9 (studio editor tools) shipped as PR #11: synonyms CRUD + hot-reload (no 43k reindex, since synonyms are query-time), per-query curations (pin/hide, applied at search time), an append-only change-history audit at /studio/relevance/history, and benchmark-before-apply (POST /synonyms/impact scores candidate synonyms against the judged set before they go live). A real bug caught in live testing: a stopword in a multi-word synonym ("chest of drawers") crashed the whole analyzer, fixed with `lenient` + an editor warning. Honest finding: broad synonyms can LOWER nDCG while raising recall. Boost rules deferred (step 7's RankingConfig already exposes field boosts; per-query/click boosts belong with step 15). Now building Step 10 (analytics & resilience) on branch `step/10-analytics-resilience`: first-party search events in Postgres (work in lite mode), studio dashboards (top queries, zero-result rate, CTR by position, latency), a judgment-driven simulated-traffic generator (honestly labeled), and the circuit breaker + Postgres FTS lite-mode fallback + status page.**
