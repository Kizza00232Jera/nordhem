...
> **STATUS (2026-06-14): Steps 1-8 merged & tagged (latest v0.8). Step 9 (editor tools) BUILT on branch `step/09-editor-tools` (8 commits): studio CRUD for synonyms (Postgres-stored, hot-reloaded into ES with NO 43k reindex, since synonyms are query-time) and curations (pin/hide per query, applied at search time, no reload); change history (audit log) at /studio/relevance/history; and benchmark-before-apply (POST /synonyms/impact scores candidate synonyms against the judged set before they go live). Caught + fixed a real bug in live testing: a stopword in a multi-word synonym ("chest of drawers") crashed the whole analyzer, fixed with lenient + an editor warning. Synonyms seeded from synonyms.txt + 9 catalog-mined groups. Honest finding: broad synonyms can LOWER nDCG while raising recall. Green: 66 search unit + 35 web unit, all typecheck + web lint; browser-tested ("davenport" -> 121 sofas via a saved synonym; pinned a lamp to #1 of "sofa"). Five per-slice teaching HTMLs (9.1-9.5) + the "two kinds of synonym" blog done. Boost rules deferred (step 7's RankingConfig already exposes boosts; per-query/click boosts belong with step 15). NEXT: PR + tag v0.9, then Step 10 (analytics & resilience).**
