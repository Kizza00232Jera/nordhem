...
> **STATUS (2026-06-15): Steps 1-9 merged & tagged (main at v0.9). Step 9 (studio editor tools) shipped as PR #11: synonyms CRUD + hot-reload (no 43k reindex, since synonyms are query-time), per-query curations (pin/hide, applied at search time), an append-only change-history audit at /studio/relevance/history, and benchmark-before-apply (POST /synonyms/impact scores candidate synonyms against the judged set before they go live). A real bug caught in live testing: a stopword in a multi-word synonym ("chest of drawers") crashed the whole analyzer, fixed with `lenient` + an editor warning. Honest finding: broad synonyms can LOWER nDCG while raising recall. Boost rules deferred (step 7's RankingConfig already exposes field boosts; per-query/click boosts belong with step 15). Step 10 (analytics & resilience) BUILT on branch `step/10-analytics-resilience` (5 slices, pending PR + tag v0.10): first-party search events in Postgres (search_performed / result_clicked / zero_result, fired via sendBeacon so they survive lite mode), a /studio/analytics dashboard (top queries, zero-result rate + backlog, CTR by position, latency p50/p95/p99, live-vs-synthetic split), a judgment-driven simulated-traffic generator (position-biased cascade click model, seeded, honestly labelled synthetic), and the circuit breaker (800ms timeout, opens after 2 failures, half-open probe) + Postgres FTS lite-mode fallback + honest banner + /status page. Bug caught in live browser testing: AND-only FTS returned 0 for "oak bed", fixed to OR semantics (176). Green: 65 web unit + 4 new real-PG integration (events/analytics/fts) + 11 search-service simulate unit; typecheck + web lint clean. NEXT: PR + tag v0.10, then Step 11 (learning loop + AI).**
...
### ✅ Step 10 — Analytics & resilience (done 2026-06-15)
...
*Teaching: search KPIs; position bias; circuit breakers & graceful degradation; query profiling.* (teaching HTML pending)
