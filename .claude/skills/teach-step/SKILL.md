...
- **One column width for everything (Antonio's preference):** `.wrap{max-width:960px}` — prose, code blocks, quiz cards, and editors all share the same width, nothing breaks out and nothing is narrower. `pre` blocks are plain full-width blocks inside it (no margin/transform tricks), font 14px, `overflow-x: auto` only as the rare-long-line fallback. Keep snippet lines under ~110 chars; split longer ones.
