...
- Each exercise: a dark monospace `<textarea>` editor (Tab inserts two spaces), **Run tests**, **Hint**, **Reset**, and **Ask the tutor about my code** buttons.
- **Editor sizing (Antonio's feedback):** editors AUTO-GROW to fit their content — `autosize()` sets height to scrollHeight on load, input, Tab, hint-insert, and reset; min-height 320px; `overflow-y: hidden` (the page scrolls, never the editor); manual `resize: vertical` kept as the override. Long lines WRAP (`white-space: pre-wrap`, `overflow-x: hidden`) — no sideways scrolling ever. Font 14.5px.
...
- **Page context (the tutor can SEE the page):** build `PAGE_CONTEXT` once at load — `.wrap` innerText (capped ~33k chars) plus every SVG serialized as `PICTURE N ("aria-label"): … labels in order: <svg textContent>` — and append it to the system prompt on every chat request. Each question is also prefixed by `askExtras()`: the `h2` section currently on screen plus any text the student has highlighted (`window.getSelection()`). Copy `buildPageContext`/`askExtras` from step-01.
- **Research tools:** chat requests send `tools: ["WebSearch", "WebFetch"]` (the server allowlists exactly these and pre-approves them via `--allowedTools`; hint requests send none). The system prompt must state capabilities HONESTLY: it can see the whole page including the PICTURE descriptions; it can search/fetch the web (prefer official docs, cross-check version numbers across sources); it has NO file access, NO bash; it must never write a pretend tool call as text and never end a reply promising to "go look and report back" — every reply complete in itself. (The step-1 tutor once hallucinated a bash call and died mid-promise; these clauses are why.)
...
- Code blocks: dark panel, simple inline token highlighting (keywords/strings/comments via spans you emit yourself — no highlight.js). `pre` blocks BREAK OUT wider than the prose column (`width: min(92vw, 960px)` centered via `margin-left: 50%; transform: translateX(-50%)`; reset in `@media print`), font 14px — long code lines should rarely need horizontal scrolling, and prose stays at 76ch. Keep snippet lines under ~110 chars; split longer ones.
